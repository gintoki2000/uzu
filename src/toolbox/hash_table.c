...
static HashTableEntry* hash_table_entry_create(void* key, void* value, u32 hashCode)
{
...
static HashTableEntry* hash_table_entry_copy(const HashTableEntry* other)
{
...
static void entry_list_insert(HashTableEntry** headRef, HashTableEntry* entry)
{
  if (*headRef == NULL)
  {
    *headRef = entry;
...
static HashTableEntry* entry_list_find(HashTableEntry*  head,
                                       void*            key,
                                       u32              hashCode,
                                       EqualFunc        equalFunc,
...
static void
entry_list_remove(HashTableEntry** headRef, HashTableEntry* entry, HashTableEntry* prevEntry)
{
  if (entry == *headRef)
    *headRef = entry->next;
  else
...
INLINE double hash_table_loadfactor(HashTable* self)
{
...
void hash_table_free_null(HashTable** table)
{
...
void hash_table_insert(HashTable* self, void* key, void* value)
{
  ...
  if ((entry = entry_list_find(self->entries[index], key, hashCode, self->equalFunc, NULL)))
  {
    entry->value = value;
  }
  else
  {
    entry_list_insert(&self->entries[index], hash_table_entry_create(key, value, hashCode));
    ++self->count;
  }
...
BOOL hash_table_remove(HashTable* self, void* key)
{
  ...
  hashCode = self->hashFunc(key);
  index    = hashCode % self->size;
  if ((entry = entry_list_find(self->entries[index], key, hashCode, self->equalFunc, &prevEntry)))
  {
    entry_list_remove(&self->entries[index], entry, prevEntry);
    if (self->destroyFunc)
      self->destroyFunc(entry->value);
    SDL_free(entry);
    --self->count;
    return TRUE;
  }
...
void hash_table_print(HashTable* ht, void (*entryToStringFunc)(void* k, void* v, char* buff))
{
  ...
  for (u32 i = 0; i < ht->size; ++i)
  {
    ...
    while (it)
    {
      entryToStringFunc(it->key, it->value, buff);
      printf("%s->", buff);
      it = it->next;
    }
    printf("NULL\n");
